...
def load_mat(path):
    ...
    # plt.imshow(Out[:,1,0].reshape(2000,2000))
    # plt.axis('off')
    return mat_contents
...
im = load_image('./irregular_inclusion_data/2000x2000.mat')
f = h5py.File('./irregular_inclusion_data/dns_fft_out_100.mat', 'r')
arrays = {}
...
model = ExpandConv(num_patch_1d=num_patch_1d, im_input = im,  ew_length = ew_length, stride=stride, BATCHSIZE = BS, weights_path = "./checkpoints/irregular_constitutive_BS256.h5")
...
# checkpoint
filepath="./checkpoints/irregular_constitutive_BS256.h5"
# 有一次提升, 则覆盖一次.
