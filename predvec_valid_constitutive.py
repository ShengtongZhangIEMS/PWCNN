...
#%%
im = load_image('./valid/im_valid.mat')
im = im['newMS']
f = h5py.File('./valid/dns_fft_out_100_valid.mat', 'r')
arrays = {}
for k, v in f.items():
    arrays[k] = np.array(v)
Out = arrays['Out']
...
stress_max, strain_max = 394.7850661988291, 0.12671173800652916
print(stress_max, strain_max)
...
model = ExpandConv(num_patch_1d=1, im_input = im, ew_length = ew_length, stride=stride, BATCHSIZE = BS,\
  weights_path = "./checkpoints/silica_constitutive_BS256.h5")
...
def constitutive(strain, model_mat, C, im_inner):
  # im_inner: 1800*1800
  # C: stiffness matrix in particle phase
  stress_par = np.dot(strain.reshape((-1,6)), C)
  stress_mat = model_mat.predict(strain.reshape((-1,6)), batch_size = 256, verbose = 2)
  stress_pred = im_inner.reshape((-1,1))*stress_par + (1-im_inner.reshape((-1,1)))*stress_mat
  return stress_pred.reshape(strain.shape)
#%%
...
y_pred_2d = np.zeros((num_load, 1800,1800, 12))
stress_pred_const = np.zeros((num_load, 1800,1800, 6))
y_pred_vec_mat = np.zeros((num_load, matrix_num, 12))
y_pred_vec_par = np.zeros((num_load, particle_num, 12))
y_true_vec_mat = np.zeros((num_load, matrix_num, 12))
y_true_vec_par = np.zeros((num_load, particle_num, 12))


output_file = './outputs/outputs_valid_1206.txt'
# output_file = 'tmp.txt'
...
# np.savez('./save_data/pred_silica_constitutive_strain_xx.npz', true = y_true_2d_iter, pred = y_pred_2d_iter,\
#   rel_err = rel_err, equ_err = equ_err, equilibrium_err = equilibrium_err)

#%%
equilibrium_err = data['equilibrium_err']
for i in range(0, 10, 2):
    plt.plot(range(1,6), equilibrium_err[i, :]/ss[0,i], '-o', label = 'load = %.3f' % ee[0, i])
